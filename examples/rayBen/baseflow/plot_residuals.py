...
def parse_log_file(log_file="lightkrylov.log"):
    ...
    with open(log_file, "r") as f:
        for line in f:
            ...
            if newton_match:
                ...
                gmres_data[current_newton_step] = {"steps": [], "residuals": [], "tolerance": tolerance}
                continue
    ...
    return {"newton_steps": newton_steps, "newton_residuals": newton_residuals, "gmres_data": gmres_data}
...
def plot_basic_residuals(data, output_file="residual.png", logscale=True):
    ...
    for newton_step, step_data in sorted(gmres_data.items()):
        ...
        ax2.plot(step_data["steps"], step_data["residuals"], marker=markers[marker_idx], color=colors[color_idx], linewidth=1.5, markersize=7)

        # Add tolerance line
        if "tolerance" in step_data:
            ax2.axhline(y=step_data["tolerance"], color=colors[color_idx], linestyle="--", alpha=0.5)

        # Create custom legend handle
        patch = mpatches.Patch(color=colors[color_idx], label=f"Newton Step {newton_step}")
...
def plot_quadratic_analysis(data, output_file="residual_quadratic.png", logscale=True):
        ...
        if len(newton_residuals) > 2:
            ...
            if len(x_fit) >= 3:
                try:
                    ...
                    ax1.loglog(x_dense + min(newton_steps), y_fitted, "g-", linewidth=1.5, label=f"Fitted: r ≈ {10**a_fit:.2e} × {fitted_rate:.4f}ⁿ")
                    ...
                    ax1.text(0.05, 0.05, f"Fitted rate: {fitted_rate:.4f}\nClassification: {convergence_class}\nLin≈0.1, Quad≈0.01, Super≈0.001", transform=ax1.transAxes, fontsize=10, bbox=dict(facecolor="white", alpha=0.8))
                except:
    ...
    if len(newton_residuals) > 1:
        ...
        if len(res_current) >= 3:
            try:
                ...
                ax2.loglog(x_ref, y_fitted, "g-", linewidth=1.5, label=f"Fitted: r₊₁ ≈ {a_fit:.2e} · rₙᵖ, p={b_fit:.4f}")
                ...
                ax2.text(0.05, 0.05, f"Fitted order: p = {b_fit:.4f}\nR² = {r_value**2:.4f}\nClassification: {convergence_type}", transform=ax2.transAxes, fontsize=10, bbox=dict(facecolor="white", alpha=0.8))
            except:
                # If fitting fails, just continue without the fit
                pass
    ...
    if len(newton_residuals) > 2:
        ...
        if convergence_rates:
            ...
            ax3.axhline(y=avg_rate, color="m", linestyle="-", linewidth=1.5, label=f"Average order: p = {avg_rate:.4f}")

            # Classify average convergence
            avg_convergence_type = classify_convergence(avg_rate)

            # Annotate with the statistics
            ax3.text(0.05, 0.05, f"Average order: p = {avg_rate:.4f}\nRange: [{min(convergence_rates):.2f}, {max(convergence_rates):.2f}]\nClassification: {avg_convergence_type}", transform=ax3.transAxes, fontsize=10, bbox=dict(facecolor="white", alpha=0.8))
