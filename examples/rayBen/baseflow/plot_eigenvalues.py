...
for npy_file in npy_files:
    try:
        ...
        if data.ndim == 2:
            ...
            plt.stem(np.arange(len(real_parts)), real_parts, linefmt="b-", markerfmt="bo", basefmt="k-", label=f"From {os.path.basename(npy_file)}")

            # Add value labels
            for i, v in enumerate(real_parts):
                plt.annotate(f"{v:.6f}", (i, v), xytext=(0, 5), textcoords="offset points", ha="center", fontsize=10)
...
if os.path.exists(filename):
    try:
        ...
        if data:
            ...
            sc = plt.scatter(real_parts, imag_parts, c=residuals, cmap="plasma", s=100, marker="x", label=f"From {filename}")
