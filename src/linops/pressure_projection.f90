      ...
      ! Solve linear system D @ D^T x = dpr
                  opts = cg_dp_opts(maxiter = 100, if_print_metadata = .true.)
                  meta = cg_dp_metadata()
                  call cg(self%DDT, dpr, x, info, rtol=rtol_dp, atol=atol_dp, options=opts, meta=meta)
